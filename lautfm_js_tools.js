...
(function() {
  ...
  if( !window.laut.fm ){
    ...
    var with_lautfm_time_offset = function(callback){
      var offset;
      if(offset = parseInt(getCookie('__lautfm__offset__'))){
        callback(offset);
      }else{
        getTime(function(t){
          offset = parseInt(new Date - t); // positive means, we're ahead of the server
    ...
    var reviver = function(key, value){
      ...
      if( key=='' || at_key ){ // started_at or toplevel (for getTime)
        if (typeof value === 'string') {
          ...
          if (a) { return new Date(Date.UTC(+a[1], +a[2] - 1, +a[3], (a[7]=='+' ? +a[4]-a[8] : +a[4]+a[8]), +a[5], +a[6])); }
        }
      }
      return value;
    };
    ...
    var apiget = function (url, callback, watch, self) {
