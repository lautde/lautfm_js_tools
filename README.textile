...
Experimentell kann @CALLBACK_OR_OPTIONS@ auch ein Options-Hash sein:

<pre><code>{container:CONTAINER, template:TEMPLATE, callback:CALLBACK}
</code></pre>

@CONTAINER@ und @TEMPLATE@ können Dom-Elemente oder HTML IDs sein. Ein @CALLBACK@ ist dann optional.

h3. Allgmeine Resourcen

<pre><code>laut.fm.time(CALLBACK_OR_OPTIONS)</code></pre>
...
<pre><code>laut.fm.stations.genre(NAMES, CALLBACK_OR_OPTIONS)</code></pre>
...
* "Experimental Template Support":http://lautde.github.com/lautfm_js_tools/station.current_song_template.html
...
* "Experimental Template Support":http://lautde.github.com/lautfm_js_tools/station.last_songs_template.html
...
<pre><code>laut.fm.station(NAME).network(CALLBACK_OR_OPTIONS)</code></pre>
...
* 2011-11-23 0.3.0 added experimental templating support
